...
def clean_file(file):
    ...
    # Remove unwanted newlines and extra headers if needed
    cleaned_content = content.replace('\r\n', '\n').strip()
    # Return the cleaned content as a BytesIO object
...
def send_data_to_api(df):
    ...
    # Determine the final response code based on success rate
    if success_count == total_requests:
        return 200  # All requests were successful
    elif success_count > 0:
        return 207  # Mixed success
    else:
        return 500  # All requests failed
...
def start_prediction(request):
    ...
    # Save plots to a buffer and encode in base64
    buffer = io.BytesIO()
    ...
    # Save final plot to a buffer and encode in base64
    buffer = io.BytesIO()
    plt.savefig(buffer, format='png')
    plt.close()
    buffer.seek(0)
    ...
    responses = send_predictions_to_api(Y_test, Y_pred)
